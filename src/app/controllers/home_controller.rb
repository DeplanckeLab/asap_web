...
class HomeController < ApplicationController
  ...
  def openapi_spec
    ...
    api_base = EnvHelpers.openapi_api_base
    spec['servers'] = [{ 'url' => api_base }]
    substitute_openapi_api_base!(spec, api_base)
  ...
  def substitute_openapi_api_base!(node, api_base)
    case node
    when Hash
      node.each { |key, value| node[key] = substitute_openapi_api_base!(value, api_base) }
    when Array
      node.map! { |value| substitute_openapi_api_base!(value, api_base) }
    when String
      node.gsub('__ASAP_API_BASE__', api_base)
    else
      node
    end
  end
