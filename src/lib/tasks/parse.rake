...
#        output_json = tmp_dir + "output.json"                                                                                                                                                
        ...
        if exit_status != 0
          logger.error("[ParseRake] Python parsing command failed with exit code #{exit_status}")
          logger.error("[ParseRake] Command output: #{output_stdout}")
          
          # Try to read error from output.json if it exists
          if File.exist?(output_json)
            ...
            if h_output['error'] || h_output['displayed_error']
              ...
              logger.error("[ParseRake] Python parsing error from output.json: #{error_msg}")
            end
          end
          exit 1
        end
        
        # Check if output.loom was created
        unless File.exist?(output_file)
          logger.error("[ParseRake] Python parsing completed but output.loom not found at #{output_file}")
          logger.error("[ParseRake] Command exit status: #{exit_status}")
          logger.error("[ParseRake] Command output length: #{output_stdout.length} chars")
          logger.error("[ParseRake] Command output (first 500 chars): #{output_stdout[0..500]}")
          
          # Check for SLURM error file if it exists
          slurm_err_file = tmp_dir + "slurm_#{run.id}.err"
          if File.exist?(slurm_err_file)
            err_content = File.read(slurm_err_file)
            logger.error("[ParseRake] SLURM error file content: #{err_content[0..1000]}")
          end
          
          # Read or create output.json with error information
          h_output = {}
          if File.exist?(output_json)
            h_output = Basic.safe_parse_json(File.read(output_json), {})
            logger.error("[ParseRake] output.json exists but contains no error field. Content: #{h_output.to_json}")
          end
          
          # Write error to output.json so SlurmJobMonitorJob can detect it
          error_msg = "Python parsing script completed but failed to create required output.loom file"
          if exit_status != 0
            error_msg = "Python parsing script failed with exit code #{exit_status}. #{error_msg}"
          end
          if output_stdout && !output_stdout.strip.empty?
            error_msg += ". Command output: #{output_stdout[0..200]}"
          end
          h_output['displayed_error'] = [error_msg]
          h_output['error'] = error_msg
          
          File.open(output_json, 'w') do |f|
            f.write(h_output.to_json)
          end
          logger.error("[ParseRake] Wrote error to output.json: #{error_msg}")
          
          exit 1
        end
        
        # Create project cell set (metadata loading will be done by finish_run)
        logger.info("[ParseRake] Creating project cell set for project #{project.key}")
        Basic.upd_project_cell_set(project)
        project.reload
        if project.project_cell_set
          logger.info("[ParseRake] Project cell set created: #{project.project_cell_set.key}")
        else
