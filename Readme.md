...
3. **Data directories** on the host (e.g. `/data/asap2`, `/data/asap2_test`) for uploads, Loom outputs, and user sandboxes.
4. **The asap_run image** — built from the companion repository at `/srv/asap_run_new` (see comments in `docker-compose.test.yml`). Rebuild after changing Python analysis scripts:
   ```bash
