...
2. **Discovery service (Mode A):** Given **`project_id`** + **`cell_set_id`** (or key), list **other projects** with the same **`project_cell_set_id`** / cell-set identity where the user is **`readable?`**, and list **compatible** `Annot` candidates. **Done:** `GET /projects/:id/discover_metadata_import_sources?cell_set_id=...` or `?cell_set_key=...` (requires **read** on target project); **`MetadataImportModeADiscoveryService`** returns sources with **`dimensions_aligned`**, per-annot **`compatible`** / **`incompatibility_reason`**, and **`import_name_allowed_on_target`** via **`MetadataNameAuthorizationService`**. **Next (item 4):** wire UI / import wizard to this endpoint.
3. **Explicit project picker (Mode B):** Given **`readable?`** source **`project_id`**, list **compatible** metadata for multi-select import (**R-M1b–R-M1c**). **Done:** `GET /projects/:id/discover_metadata_import_from_project?source_project_id=...` or `?source_project_key=...` (read on target); **`MetadataImportModeBDiscoveryService`** enforces **`readable?`** on source, same **`project_cell_set_id`**, and returns a flat **`annots`** list with the same compatibility fields as Mode A. Shared logic: **`MetadataImportDiscoveryHelpers`**. **Next (item 4):** picker UI + import execution.
