...
#  exit
  ...
  url = "https://s3ftp.flybase.org/releases/current/precomputed_files/genes/fbgn_fbtr_fbpp_expanded_fb_#{h_tool_versions['flybase_genes']}.tsv.gz"
  cmd = "wget -qO - '#{url}' | zcat"
  res = `#{cmd}`
  res.split("\n").select{|l| !l.match(/^\s*\#/) and !l.empty?}.each do |l|
    t = l.split("\t")
  ...
  url = "https://s3ftp.flybase.org/releases/current/precomputed_files/genes/gene_map_table_fb_#{h_tool_versions['flybase_genes']}.tsv.gz"
  cmd = "wget -qO - '#{url}' | zcat"
  res = `#{cmd}`
  res.split("\n").select{|l| !l.match(/^\s*\#/) and !l.empty?}.each do |l|
    t = l.split("\t")
  ...
  url = "https://s3ftp.flybase.org/releases/current/precomputed_files/genes/gene_snapshots_fb_#{h_tool_versions['flybase_genes']}.tsv.gz"
  cmd = "wget -qO - '#{url}' | zcat"
