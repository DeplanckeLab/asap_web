...
**Major version** is incremented when schema updates are incompatible with the Loom data encoding or CELLxGENE API(s). Examples include:
...
* [`/matrix` (Matrix layers)](#matrix-layers), which describe the data required for different assays
* [`/row_attrs/` (Cell metadata)](#row_attrs-cell-metadata), which describe each cell in the dataset
* [`/row_attrs/` (Embeddings)](#row_attrs-embeddings), which describe each embedding in the dataset
* [`/row_attrs/` (Pairwise annotations)](#row_attrs-pairwise), which describe pairwise annotation of observations
* [`/col_attrs/` (Gene metadata)](#col_attrs-gene-metadata), which describe each gene in the dataset
* [`/col_attrs/` (Multi-dimensional annotations)](#col_attrs-multidimensional), which describe multi-dimensional annotation of variables/features
* [`/col_attrs/` (Pairwise annotations)](#col_attrs-pairwise), which describe pairwise annotation of variables/features
* [Global attributes (Dataset metadata)](#global-attributes-dataset-metadata), which describe the dataset as a whole

## General Requirements

**Loom.** The canonical data format for CELLxGENE Discover is HDF5-backed [Loom](http://loompy.org/) format. The on-disk format must follow the [Loom file format specification](http://linnarssonlab.org/loompy/format/index.html). Part of the rationale for selecting this format is to allow CELLxGENE to access both the data and metadata within a single file. The schema requirements and definitions for the Loom `/matrix`, `/row_attrs/`, `/col_attrs/`, `/layers/`, and global attributes are described below.
...
**Unique Names**. The names of schema and data submitter metadata fields in `/row_attrs/` and `/col_attrs/` MUST be unique. For example, duplicate `"feature_biotype"` keys in `/col_attrs/` are not allowed.
...
*   Any genes that publishers wish to filter from the normalized matrix MAY have their values replaced by zeros and MUST be flagged in the attribute [`feature_is_filtered`](#feature_is_filtered) of [`/col_attrs/`](#col_attrs-gene-metadata), which will mask them from exploration.
...
|-|-|-|-|-|
| scRNA-seq (UMI, e.g. 10x multiome, 10x v3, Slide-seqV2) | REQUIRED. Values MUST be de-duplicated molecule counts. Each cell MUST contain at least one non-zero value. All non-zero values MUST be positive integers stored as `numpy.float32`. Any two cells MUST NOT contain identical values for all their features. | `/layers/raw` unless no "normalized" is provided, then `/matrix` | STRONGLY RECOMMENDED | `/matrix` |
| Visium Spatial (e.g. V1, CytAssist) | REQUIRED. Values MUST be de-duplicated molecule counts. All non-zero values MUST be positive integers stored as `numpy.float32`.<br><br>If the global attribute <code>spatial/is_single</code> is <code>False</code> then each cell MUST contain at least one non-zero value.<br><br>If the global attribute <code>spatial/is_single</code> is <code>True</code> then the unfiltered feature-barcode matrix (<code>raw_feature_bc_matrix</code>) MUST be used. See <a href="https://www.10xgenomics.com/support/software/space-ranger/analysis/outputs/space-ranger-feature-barcode-matrices">Space Ranger Feature-Barcode Matrices</a>.<br><br>if <code>assay_ontology_term_id</code> is <a href="https://www.ebi.ac.uk/ols4/ontologies/efo/classes?obo_id=EFO%3A0022860"><code>"EFO:0022860"</code></a> for <i>Visium CytAssist Spatial Gene Expression, 11mm</i>, this matrix MUST contain 14336 rows; otherwise, this matrix MUST contain 4992 rows.<br><br>If the <code>/row_attrs/in_tissue</code> value is <code>1</code>, then the cell MUST contain at least one non-zero value and any two cells MUST NOT contain identical values for all their features.<br><br>If any <code>/row_attrs/in_tissue</code> values are <code>0</code>, then at least one cell corresponding to a <code>/row_attrs/in_tissue</code> with a value of <code>0</code> MUST contain a non-zero value.| `/layers/raw` unless no "normalized" is provided, then `/matrix` | STRONGLY RECOMMENDED | `/matrix` |
...
| unpaired Accessibility (e.g. ATAC-seq, mCT-seq) | NOT REQUIRED | | REQUIRED | `/matrix` | STRONGLY RECOMMENDED |
|||||
...
## `/row_attrs/` (Cell Metadata)

Cell metadata is stored in `/row_attrs/` as arrays with one value per cell/observation.

Curators MUST annotate the following attributes in `/row_attrs/`:

### Cell/Observation Identifiers

<table><tbody>
    <tr>
      <th>Key</th>
      <td>cell/observation identifiers</td>
    </tr>
    <tr>
      <th>Annotator</th>
    ...
    <tr>
      <th>Value</th>
        <td><code>str</code> array. The `/row_attrs/CellID` or similar identifier array MUST contain unique identifiers for observations.<br><br></td>
    </tr>
</tbody></table>
<br>
...
<table><tbody>
    ...
    <tr>
      <th>Annotator</th>
         <td>Curator MUST annotate if <code>assay_ontology_term_id</code> is  a descendant of <a href="https://www.ebi.ac.uk/ols4/ontologies/efo/classes?obo_id=EFO%3A0010961"><code>"EFO:0010961"</code></a> for <i>Visium Spatial Gene Expression</i> and the global attribute <code>spatial/is_single</code> is <code>True</code>; otherwise, this key MUST NOT be present.</td>
    </tr>
    <tr>
      <th>Value</th>
  ...
  <tr>
    ...
    <td>
      ...
      <ul>
        ...
        <li>
          <code>assay_ontology_term_id</code> is a descendant of <a href="https://www.ebi.ac.uk/ols4/ontologies/efo/classes?obo_id=EFO%3A0010961"><code>"EFO:0010961"</code></a> for <i>Visium Spatial Gene Expression</i>,<br>the global attribute <code>spatial/is_single</code> is <code>True</code>,<br>and the corresponding value of <code>/row_attrs/in_tissue</code> is <code>0</code>
        </li>
      </ul>
...
<table><tbody>
    ...
    <tr>
      <th>Annotator</th>
      <td>Curator MUST annotate if the global attribute <code>genetic_perturbations</code> is present; otherwise this key MUST NOT be present.</td>
    </tr>
    <tr>
      <th>Value</th>
        <td>
         ...
         The value MUST be either be <code>"na"</code> or one or more genetic perturbation identifiers in ascending lexical order separated by the delimiter <code>" || "</code> with no duplication of identifiers. Each identifier MUST match a key in the global attribute <code>genetic_perturbations</code>. If the corresponding value of <code>genetic_perturbation_strategy</code> is <code>"control"</code>, then the value of the role in <code>genetic_perturbations[<i>id</i>]/role</code> for each matching identifier MUST be <code>"control"</code>.<br><br>All observations MUST NOT contain <code>"na"</code>.
        </td>
    </tr>
</tbody></table>
<br>
...
<table><tbody>
    ...
    <tr>
      <th>Annotator</th>
      <td>Curator MUST annotate if <code>/row_attrs/genetic_perturbation_id</code> is present; otherwise, this key MUST NOT be present</td>
    </tr>
    <tr>
      <th>Value</th>
        <td><code>str</code>. If the corresponding value of <code>/row_attrs/genetic_perturbation_id</code> is <code>"na"</code>, then the value MUST be <code>"no perturbations"</code>; otherwise, the value MUST be one of:
          <ul>
...
<table><tbody>
    ...
    <tr>
      <th>Value</th>
        <td><code>bool</code>. This MUST be <code>False</code> if the global attribute <code>spatial/is_single</code> is <code>False</code>. This MUST be <code>True</code> if this is the canonical instance of this cellular observation and <code>False</code> if not. This is commonly <code>False</code> for meta-analyses reusing data or for secondary views of data.
        </td>
    </tr>
</tbody></table>
<br>
...
When a dataset is uploaded, CELLxGENE Discover MUST automatically add the matching human-readable name for the corresponding ontology term to `/row_attrs/`. Curators MUST NOT annotate the following attributes.
...
When a dataset is uploaded, CELLxGENE Discover MUST annotate a unique observation identifier for each cell. Curators MUST NOT annotate the following attribute.
...
## `/row_attrs/` (Embeddings)


The value for each embedding attribute MUST be a  `numpy.ndarray` of shape `(n_obs, m)`, where `n_obs` is the number of rows in `/matrix` and `m >= 1`. 

To display a dataset in CELLxGENE Explorer, Curators MUST annotate **one or more** embeddings of at least two-dimensions (e.g. tSNE, UMAP, PCA, spatial coordinates) as `numpy.ndarrays` in `/row_attrs/`.<br><br>
...
<table><tbody>
    ...
    <tr>
      <th>Annotator</th>
      <td>Curator MUST annotate if the global attribute <code>spatial/is_single</code> is <code>True</code>.<br><br>Curator MAY annotate if the global attribute <code>spatial/is_single</code> is <code>False</code>.
      ...
      <th>Value</th>
        <td><code>numpy.ndarray</code> with the following requirements<br><br>
          <ul>
          <li>MUST have the same number of rows as <code>/matrix</code> and MUST include at least two dimensions</li>
          ...
          <li>MUST NOT contain <a href="https://numpy.org/devdocs/reference/constants.html#numpy.nan">Not a Number (<code>numpy.nan</code>)</a> values</li></ul><br>If <code>assay_ontology_term_id</code>is a descendant of <a href="https://www.ebi.ac.uk/ols4/ontologies/efo/classes?obo_id=EFO%3A0010961"><code>"EFO:0010961"</code></a> for <i>Visium Spatial Gene Expression</i> and the global attribute <code>spatial/is_single</code> is <code>True</code>, the array MUST be created from the corresponding <code>pxl_row_in_fullres</code> and <code>pxl_col_in_fullres</code> fields from <code>tissue_positions_list.csv</code> or <code>tissue_positions.csv</code>. See <a href="https://www.10xgenomics.com/support/software/space-ranger/analysis/outputs/spatial-outputs">Space Ranger Spatial Outputs</a>.
        </td>
    </tr>
</tbody></table>
<br>
...
<table><tbody>
    <tr>
      ...
      {suffix} is presented as text to users in the <b>Embedding Choice</b> selector in CELLxGENE Explorer so it is STRONGLY RECOMMENDED that it be descriptive.<br><br>See also <code>default_embedding</code> global attribute.</td>
    </tr>
    <tr>
      <th>Annotator</th>
...
## `/row_attrs/` (Pairwise Annotations)

The size of the ndarray stored for a pairwise annotation key in `/row_attrs/` MUST NOT be zero.
<br>

## Global Attributes (Dataset Metadata)

Global attributes are stored at the root level of the Loom file. The data stored as a value for a global attribute MUST be `True`, `False`, `None`, or its size MUST NOT be zero.

Curators MUST annotate the following global attributes:
...
#Curators MAY also annotate the following optional global attributes. If the attribute is present, then its value MUST NOT be empty.
#
### batch_condition
...
<table><tbody>
  <tr>
    <th>Key</th>
      <td>
        {column}_colors where {column} MUST be the name of a categorical attribute in <code>/row_attrs/</code> that<br> is annotated by the data submitter or curator. The following attributes that are annotated by CELLxGENE<br> Discover MUST NOT be specified as {column}:<br><br>
      <ul>
        <li>assay</li>
        <li>cell_type</li>
      ...
      Instead annotate {column}_ontology_term_id_colors for these attributes such as <code>assay_ontology_term_id</code>.<br><br>
    </td>
  </tr>
  <tr>
    <th>Annotator</th>
    ...
    <th>Value</th>
      <td>
        <code>numpy.ndarray</code>. This MUST be a 1-D array of shape <code>(, c)</code>, where <code>c</code> is greater than or equal to the<br> number of categories in the {column} as calculated by:<br><br>
           <samp>/row_attrs/{column}</samp> categories size<br><br>
        The color code at the Nth position in the <code>ndarray</code> corresponds to the Nth category of <samp>/row_attrs/{column}</samp>.<br><br>For example, if <code>/row_attrs/cell_type_ontology_term_id</code> includes two categories:<br><br>
        <samp>/row_attrs/cell_type_ontology_term_id</samp> values<br><br>
...
Curators MUST NOT annotate the following global attributes.
    ...
    <tr>
      <th>Value</th>
        <td><code>str</code>. Its format MUST use the following template:
          ...
          A citation for a Loom dataset with a Publication DOI:<br><br>"<code><b>Publication:</b> https://doi.org/10.1126/science.abl4896 <b>Dataset Version:</b> https://datasets.cellxgene.cziscience.com/dbd8b789-3efa-4a63-9243-90cff64f2045.loom <b>curated and distributed by CZ CELLxGENE Discover in Collection:</b> https://cellxgene.cziscience.com/collections/e5f58829-1a66-40b5-a624-9046778e74f5"</code><br><br>
...
When a dataset is uploaded, CELLxGENE Discover MUST automatically add the `schema_version` global attribute and its value.
...
## `/col_attrs/` (Gene Metadata)

Gene metadata is stored in `/col_attrs/` as arrays with one value per gene/feature.

Curators MUST annotate the following attributes in `/col_attrs/`.
...
<table><tbody>
    <tr>
      <th>Key</th>
      <td>cell/observation identifier array</td>
    </tr>
    <tr>
      <th>Annotator</th>
      <td>Curator MUST annotate.</td>
...
Curators MUST annotate the following attribute in `/col_attrs/`:
...
<table><tbody>
    ...
    <tr>
      <th>Value</th>
        <td>
          <code>bool</code> array. When a raw matrix is not present, the value for all features MUST be <code>False</code>.<br><br>
          When both a raw and normalized matrix are present, this MUST be <code>True</code> if the feature was filtered out in the normalized matrix (`/matrix`) but is present in the raw matrix (`/layers/raw`). The value for all cells of the given feature in the normalized matrix MUST be <code>0</code>. If a feature contains all zeroes in the normalized matrix, then either the corresponding feature in the raw matrix MUST be all zeroes or the value MUST be <code>True</code>.
        <td>
    </tr>
</tbody></table>
<br>

Curators MUST NOT annotate the following attributes in `/col_attrs/`.

When a dataset is uploaded, CELLxGENE Discover MUST automatically add the matching human-readable name for the corresponding feature biotype, identifier, and the NCBITaxon term for the reference organism to `/col_attrs/`. In addition, it MUST
...
## `/col_attrs/` (Multi-dimensional Annotations)

The size of the ndarray stored for a multi-dimensional annotation key in `/col_attrs/` MUST NOT be zero.
<br>

## `/col_attrs/` (Pairwise Annotations)
The size of the ndarray stored for a pairwise annotation key in `/col_attrs/` MUST NOT be zero.
<br>
...
<table><tbody>
    ...
    <tr>
      <th>Value</th>
        <td><code>str</code>. This MUST be an observation identifier from the <a href="#cellobservation-identifiers">cell/observation identifiers</a> of the associated Dataset. Every cell/observation identifier value of the associated Dataset MUST appear at least once in this array.
        </td>
    </tr>
</tbody></table>
<br>
...
* /col_attrs/ (Gene Metadata)
  * Updated cell/observation identifier requirements to remove version suffixes from Ensembl stable identifiers
...
* General Requirements
  * Updated to use Loom file format instead of AnnData/H5AD format
* Required Ontologies
...
* General Requirements
  * Loom is now the canonical data format. The schema outline and descriptions are Loom-centric.
...
* var
  ...
  * Added requirements for gene metadata in `/col_attrs/`
