...
| `widget` | string | -- | Determines the form input type. One of: `"input_data"`, `"textfield"`, `"hidden"`, `"checkbox"`, `"select"`. Attributes with no widget or `obsolete: true` are skipped. | `_attributes.html.erb` |
...
| `min_nber_items` | integer | input_data | Minimum number of items the user must select. Rendered as a constraint badge and enforced client-side. | `_attributes.html.erb` |
...
| `not_null` | boolean | all | Marks the field as required. Renders a "required" badge. For `select`, prevents the empty/null option from appearing. | `_attributes.html.erb` |
...
| `constraints` | object | all | Validation constraints object. Currently supports `required_if`. See section 1.4. Emitted as `data-attr-constraints` for JS. | `_attributes.html.erb`, `reqs_controller.rb` |
...
The `constraints` key holds a hash. Currently one sub-key is supported:

#### `required_if`

Makes the field required only when another attribute has a specific value. Evaluated server-side in `reqs_controller.rb` `validate_required_if_constraints`.
...
```json
{
  "constraints": {
    "required_if": [
      { "attr": "some_other_attr", "equals": "some_value" }
    ]
  }
}
```
...
| `src/app/helpers/application_helper.rb` | Run badge display, `default`-based hiding |
